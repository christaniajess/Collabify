...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root@localhost:3306/account'  # Replace if needed
...
def get_users_by_interests(interests):
    try:
        users = User.query.filter(User.interests.ilike(f'%{interests}%')).all()
        user_list = []
        for user in users:
            user_data = {
                'user_id': user.user_id,
